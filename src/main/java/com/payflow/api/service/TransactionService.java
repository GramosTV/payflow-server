...
public class TransactionService {
     ...
     * @param destinationWallet   Destination wallet (can be null for external
     *                            withdrawals)
    ...
    public Transaction createTransaction(
        ...
        try {
            ...
            if (sourceWallet != null && destinationWallet != null && !sourceCurrency.equals(destinationCurrency)) {
                ...
                log.debug("Applied exchange rate of {} from {} to {}",
                        exchangeRate, sourceCurrency, destinationCurrency);
            }
            ...
            throw new PayflowApiException("Failed to create transaction", e, HttpStatus.INTERNAL_SERVER_ERROR);
        }
    }
    ...
    public Transaction createDepositTransaction(Wallet wallet, BigDecimal amount) {
        log.info("Creating deposit transaction for wallet {} with amount {}",
    ...
    public Transaction createTransferTransaction(User sender, TransactionRequest request) {
        log.info("Creating transfer transaction from {} to {} with amount {}",
                request.getSourceWalletNumber(), request.getDestinationWalletNumber(), request.getAmount());
        Wallet sourceWallet = walletRepository.findByWalletNumberWithLock(request.getSourceWalletNumber())
                .orElseThrow(() -> {
                    log.error("Source wallet not found: {}", request.getSourceWalletNumber());
                    return new ResourceNotFoundException("Wallet", "walletNumber", request.getSourceWalletNumber());
                });

        Wallet destinationWallet = walletRepository.findByWalletNumber(request.getDestinationWalletNumber())
                .orElseThrow(() -> {
                    log.error("Destination wallet not found: {}", request.getDestinationWalletNumber());
                    return new ResourceNotFoundException("Wallet", "walletNumber",
                            request.getDestinationWalletNumber());
                });

        if (!sourceWallet.getUser().getId().equals(sender.getId())) {
            log.error("User {} attempted to send money from wallet {} that they don't own",
                    sender.getId(), sourceWallet.getId());
        ...
        if (sourceWallet.getBalance().compareTo(request.getAmount()) < 0) {
            log.error("Insufficient balance in wallet {}: requested {} but available {}",
                    sourceWallet.getId(), request.getAmount(), sourceWallet.getBalance());
        ...
        try {
            ...
            if (!sourceWallet.getCurrency().equals(destinationWallet.getCurrency())) {
                ...
                convertedAmount = exchangeRateService.convertCurrency(
                        ...
                        sourceWallet.getCurrency(),
                        destinationWallet.getCurrency());
                log.debug("Applied exchange rate of {} from {} to {}, converted amount: {}",
                        exchangeRate, sourceWallet.getCurrency(), destinationWallet.getCurrency(), convertedAmount);
            }
    ...
    public Transaction createMoneyRequestTransaction(MoneyRequest moneyRequest, Wallet sourceWallet) {
        log.info("Processing money request payment from wallet {} for request ID {}",
                sourceWallet.getWalletNumber(), moneyRequest.getId());
        ...
        if (sourceWallet.getBalance().compareTo(amount) < 0) {
            log.error("Insufficient balance in wallet {}: requested {} but available {}",
                    sourceWallet.getId(), amount, sourceWallet.getBalance());
        ...
        try {
            ...
            log.info("Successfully created money request transaction with ID: {}", savedTransaction.getId());
            return savedTransaction;
    ...
    public Transaction processQRCodeTransaction(Transaction transaction) {
        log.info("Processing QR code transaction between wallets {} and {}",
    ...
    public Transaction getTransactionByNumber(String transactionNumber) {
        ...
        return transactionRepository.findByTransactionNumber(transactionNumber)
                .orElseThrow(() -> {
                    ...
                    return new ResourceNotFoundException("Transaction", "transactionNumber", transactionNumber);
                });
    }
    ...
    public List<Transaction> getUserTransactionsInDateRange(User user, LocalDateTime startDate, LocalDateTime endDate) {
        log.debug("Retrieving transactions for user ID: {} between {} and {}",

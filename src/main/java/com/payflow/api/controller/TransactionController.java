...
public class TransactionController {
  ...
  @Operation(
      ...
      description =
          "Transfer funds from one wallet to another. The source wallet must belong to the authenticated user.")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "201",
  ...
  public ResponseEntity<TransactionResponse> transferMoney(
    ...
    log.info(
        "Transfer request received from user {} for amount {}",
        currentUser.getId(),
        transactionRequest.getAmount());

    User user = userService.getUserById(currentUser.getId());
    Transaction transaction =
        transactionService.createTransferTransaction(user, transactionRequest);
  ...
  @Operation(
      ...
      description =
          "Retrieves a paginated list of all transactions where the current user is either sender or receiver")
  @ApiResponses(
      value = {
  ...
  public ResponseEntity<Page<TransactionResponse>> getMyTransactions(
    ...
    log.debug(
        "Fetching transactions for user {}, page {}, size {}",
        currentUser.getId(),
    ...
    log.debug(
        "Returned {} transactions out of {}",
        transactionResponses.getNumberOfElements(),
  ...
  @GetMapping("/summary")
  @Operation(
      summary = "Get transaction summaries for the current user",
      description =
          "Retrieves a paginated list of transaction summaries with optimized data loading")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
            description = "Transaction summaries retrieved successfully"),
        @ApiResponse(responseCode = "401", description = "User is not authenticated")
      })
  public ResponseEntity<Page<TransactionSummary>> getTransactionSummaries(
    ...
    log.debug(
        "Fetching transaction summaries for user {}, page {}, size {}",
        currentUser.getId(),
        pageable.getPageNumber(),
        pageable.getPageSize());

    User user = userService.getUserById(currentUser.getId());
    Page<TransactionSummary> summaries =
        transactionService.getUserTransactionSummaries(user, pageable);

    log.debug(
        "Returned {} transaction summaries out of {}",
  ...
  @Operation(
      ...
      description =
          "Retrieves detailed information about a specific transaction. User must be either the sender or receiver.")
  @ApiResponses(
      value = {
        @ApiResponse(responseCode = "200", description = "Transaction retrieved successfully"),
        @ApiResponse(
            responseCode = "403",
            description = "User is not authorized to view this transaction"),
  ...
  public ResponseEntity<TransactionResponse> getTransactionById(
    ...
    if (!transaction.getSender().getId().equals(currentUser.getId())
      ...
      log.warn(
          "User {} attempted to access transaction {} that they're not involved in",
          currentUser.getId(),
          transactionId);
  ...
  @Operation(
      ...
      description =
          "Retrieves all transactions for the current user within the specified date range")
  @ApiResponses(
      value = {
  ...
  public ResponseEntity<List<TransactionResponse>> searchTransactions(
    ...
    log.debug(
        "Searching transactions for user {} between {} and {}",
        currentUser.getId(),
        startDate,
        endDate);
    ...
    List<Transaction> transactions =
        transactionService.getUserTransactionsInDateRange(user, startOfDay, endOfDay);

    List<TransactionResponse> transactionResponses =
        transactions.stream().map(TransactionResponse::fromEntity).collect(Collectors.toList());
